...
export default function MyBusBookings() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await API.get('/bus-bookings/my');
  ...
  const handleCancel = async (id) => {
    ...
    setCancellingId(id);
    try {
      await API.put(`/bus-bookings/${id}/cancel`);
      fetchBookings();
    } catch (err) {
  ...
  return (
    <div className="page-container animate-fade-in">
      <div className="mb-8">
        <h1 className="page-title mb-2">🎫 My Bus Bookings</h1>
        <p className="text-dark-400">Manage your bus reservations</p>
      </div>
      ...
      {bookings.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-dark-500">Search for buses and book your first ticket!</p>
        </div>
      ) : (
          ...
          <table className="data-table">
            ...
            <tbody>
              {bookings.map((b) => (
                <tr key={b.id}>
                  <td>
                    <div className="font-medium text-white">{b.bus?.name}</div>
                    <div className="text-xs text-dark-400">{b.bus?.busNumber}</div>
                  </td>
                  <td className="text-dark-300">{b.bus?.source} → {b.bus?.destination}</td>
                  <td>
